...
def train_model(self, req_dict, model_registry_key):
    ...
    # Easy access to some fields, that we regularly update
    ...
    self.update_state(state='STARTED', meta={'status': 'Training started'})
    ...
    try:
        ...
        self.update_state(state='SUCCESS', meta={'status': 'Training completed'})
    except Exception as e:
        progress.set_status("training", JobStatusEnum.FAILED, f"Training failed! Error: {e}")
        self.update_state(state='FAILURE', meta={'status': f'Training failed: {str(e)}'})
        raise e
    finally:
