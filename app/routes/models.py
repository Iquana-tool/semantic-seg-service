...
from paths import MODEL_WEIGHTS_PATH, TRAINING_RUNS_PATH, LOG_PATH
...
async def get_trained_models_of_dataset(dataset_id: int):
    ...
    trained_models = [file for file in os.listdir(MODEL_WEIGHTS_PATH) if file.endswith(".json")]
    trained_models_result = []
    for meta_json_file in trained_models:
        name = meta_json_file.split(".")[0]
        identifier, model_id = name.split("_")
        meta_entry = json.load(open(os.path.join(MODEL_WEIGHTS_PATH, meta_json_file)))
...
async def delete_model(model_id: int):
    ...
    files = [f for f in os.listdir(MODEL_WEIGHTS_PATH) if f.split("_")[-1].split(".")[0] == str(model_id)]
    for file in files:
